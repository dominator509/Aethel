...
async fn test_regression_e2e_storage_compaction_and_routing() {
    ...
    let storage = LSMTree::new(db_path.clone(), storage::StorageEngineConfig::default())
        .await
        .unwrap();
    ...
    for i in 0..10 {
        ...
        storage.put(key.clone(), val.clone(), 0).await.unwrap();
    }

    // Test extraction
    let test_key = Bytes::from("key_5");
    let test_val = storage.get(&test_key).await.unwrap();
    assert_eq!(test_val, Some(Bytes::from("val_5")));
}
