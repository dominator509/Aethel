...
async fn test_blackbox_boundary_payload_size() {
    ...
    let target_node = AethelNode::bootstrap(addr, temp_dir.path().to_path_buf())
        .await
        .unwrap();
    ...
    let client_node = AethelNode::bootstrap(client_addr, client_dir.path().to_path_buf())
        .await
        .unwrap();
    ...
    client_node
        .network
        .broadcast_transaction(&min_payload, &[(target_addr, target_peer_id.clone())])
        .await;
    ...
    client_node
        .network
        .broadcast_transaction(&max_payload, &[(target_addr, target_peer_id.clone())])
        .await;
    ...
    client_node
        .network
        .broadcast_transaction(&excessive_payload, &[(target_addr, target_peer_id.clone())])
        .await;
    ...
    client_node
        .network
        .broadcast_transaction(&survival_payload, &[(target_addr, target_peer_id.clone())])
        .await;
...
async fn test_blackbox_workflow_emulation_state() {
    ...
    for i in 0..10 {
        ...
        client_node
            .network
            .broadcast_transaction(&payload, &[(target_addr, target_peer_id.clone())])
            .await;
    }
...
async fn test_blackbox_negative_leakage() {
    ...
    client_node
        .network
        .broadcast_transaction(&malicious_payload, &[(target_addr, wrong_peer_id)])
        .await;
